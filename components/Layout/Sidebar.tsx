...
import CollapseIcon from '../icons/CollapseIcone';
...
const menuItems = [
    ...
    {id: 2, label: "Users", icon: UsersIcon, link: "/users"},
]
...
const Sidebar = () => {
    ...
    //TODO: Check if the menu has a define type to change the any here
    const getNavItemClasses = (menu: any) => {
        return classNames("flex items.center cursor-pointer hover:bg-light-lighter rounded w-full overflow-hidden whitespace-nowrap", {
            ["bg-light-ligher"] : activeMenu?.id === menu.id
        })
    }
                        ...
                        <div className={classes}>
                            <Link href={menu.link}>
                                <a className='flex py-4 px-3 items-center w-full h-full'>
                                    ...
                                    {!toggleCollapse && (
                                        <span className={classNames('text-md font-medium text-text-light ml-3')}>{menu.label}</span>
                                    )}
                                </a>
                            </Link>
                        </div>
